...
{
    LfgProposalPlayer(): role(0), accept(LFG_ANSWER_PENDING), groupId(0) {};
    uint8 role;                                             // Proposed role
    LfgAnswer accept;                                       // Accept status (-1 not answer | 0 Not agree | 1 agree)
    uint32 groupId;                                         // Original group guid (Low guid) 0 if no original group
};
...
struct LfgProposal
{
    LfgProposal(uint32 dungeon): dungeonId(dungeon), state(LFG_PROPOSAL_INITIATING), groupId(0), leaderLowGuid(0) {}
    ...
    uint32 groupId;                                         // Proposal group (0 if new)
...
class LFGMgr
{
    ...
    private:
        void Cleaner();
        void AddGuidToNewQueue(ObjectGuid guid);
        void AddToQueue(ObjectGuid guid, LfgRolesMap* roles, LfgDungeonSet* dungeons);

        bool RemoveFromQueue(ObjectGuid guid);
