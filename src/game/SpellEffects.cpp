        ...
        switch(m_spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
            {
                switch(m_spellInfo->Id)                     // better way to check unknown
                {
                    ...
                    case 66316:                              // Spinning Pain Spike (Trial Of Crusader, Lord Jaraxxus encounter, all difficult)
                    case 67100:
                    case 67101:
                    case 67102:
                    {
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
                ...
                case 28159:                                 // Spell Shock Overload (Thaddius)
                {
                    if (!unitTarget)
                        return;

                    unitTarget->RemoveAurasDueToSpell(28160);         // Remove stun
                    unitTarget->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                    unitTarget->CastSpell(unitTarget, 28136, true);   // cast Lightning Effect
                    break;
                }
                case 28136:                                 // Spell Lightning Visual (Thaddius)
                {
                    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT)
                        return;
                ...
                case 55804:                                 // Healing Finished (triggered by item spell Telluric Poultice)
                {
                    ...
                    switch(urand(0,3))
                    {
                        case 0: unitTarget->MonsterSay("Let us fight the Irons together!",LANG_UNIVERSAL);
                            break;
                        case 1: unitTarget->MonsterSay("Thank you! I thought I was doomed.",LANG_UNIVERSAL);
                            break;
                        case 2: unitTarget->MonsterSay("Let me fight by your side!",LANG_UNIVERSAL);
                            break;
                        case 3: unitTarget->MonsterSay("I was certain I was going to die out here.",LANG_UNIVERSAL);
                            break;
                    }
                ...
                case 66181:                                 // Anub'arak Find Never Cold and Cast Ice Spikes
                {
                    ...
                    m_caster->CastSpell(m_caster, 67732, true);
                    ...
                    m_caster->CastSpell(m_caster, 65920, true);
                    return;
                }
                case 67322:                                 //Burrower Cast (Nerubian Burrower - Toc10)
                {
...
void Spell::EffectTransmitted(SpellEffectIndex eff_idx)
{
    ...
    else
    {
        ...
        if (goinfo->type == GAMEOBJECT_TYPE_FISHINGNODE)
        {
            ...
            if (!m_caster->GetTerrain()->IsInWater(fx, fy, waterZ - 1.0f, &liqData))
            {
