...
int WorldSocket::HandleAuthSession(WorldPacket& recvPacket)
{
    ...
    if (fields[12].GetUInt32() != NULL && fields[12].GetUInt32() == id)                 // check to see if account has forced perms
    {
        if (fields[13].GetUInt32() != NULL && fields[13].GetUInt32() == realmID)        // check to see if account has forced perms on realm
            security = fields[14].GetUInt32();                                          // if it does, applies forced perms
        else
